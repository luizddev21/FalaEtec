...
export async function getUserImp({ sub: rm, type }) {
    let classroom = "";
...
export async function getAllUserImp({ sub: rm, type }) {
...
export async function registerUserImp({ rm, password, name, turma_id, type }) {
...
export async function changeUserPasswordImp({ rm, newPassword, type }) {
    if (!['aluno', 'professor', 'gestor'].includes(type)) {
        throw new Error('Tipo de usuário inválido');
    }
...
export async function deleteUserImp({ rm, type }) {
    const sql = `
        DELETE
        FROM ${type}
        WHERE ${type}_id = ?
